...
function getChildCategory($categories, $category_parent_id,$selected_category,$name)
{
    ...
    foreach ($categories as $category) {
        if($index == 0){
            ...
            echo "<a data-id='' href='#' onclick='window.location.href=\"".Url::to('/fr/search?q=&category='.$category['parent_ids'])."\"'  class='filter $selected_class' data-toggle='collapse' data-parent-ids='' data-parent='#' data-type='category-filter'> ".$title_all." </a> 
                  ";  
            echo '</div>';
            $index++;
        }
